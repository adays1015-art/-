...
export default function UpcycleItemsClient({ initial }: { initial: Item[] }) {
  ...
  const canEditItems = useCanEdit("upcycle-items");
  const { save, saving, error: saveError, clearError } = useResourceSave("/api/upcycle-items");
  ...
  async function onSave() {
    ...
    try {
      const refetch = await fetch("/api/upcycle-items");
  ...
  return (
    <div>
      <PageHeader
        title="업사이클 품목마스터"
        description="업사이클 라인 전용 품목. 버려지는 화장품으로 만드는 별도 제품군입니다."
        actions={
          <button
