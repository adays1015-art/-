...
export default function UpcycleMaterialsClient({
  ...
  const canEditMat = useCanEdit("upcycle-materials");
  ...
  const { save, saving, error: saveError, clearError } = useResourceSave("/api/upcycle-materials");
  ...
  async function onSave() {
    ...
    fetch("/api/upcycle-materials", { cache: "no-store" })
      .then((r) => r.json())
  ...
  return (
    <div>
      <PageHeader
        title="업사이클 원료재고"
        description="버려지는 화장품 등 업사이클 전용 원재료. 업사이클 생산 시 BOM 비율로 차감됩니다."
        actions={
