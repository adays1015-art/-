...
export default function ItemProductionClient({
  ...
  // 라벨 인쇄 모달 — 보관 용기에 붙이는 LOT 라벨 (50 × 20 mm).
  ...
  const canEditProd = useCanEdit("item-production");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/item-production?probe=headers", { cache: "no-store" });
  ...
  async function submitDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/item-production?id=${encodeURIComponent(deleteTarget.id)}`, {
        method: "DELETE",
      });
  ...
  async function submitDispose() {
    ...
    setDisposeError(null);
    try {
      const res = await fetch("/api/item-production", {
        method: "POST",
  ...
  async function onSave() {
    ...
    if (!isNew) {
      ...
      try {
        ...
        const exResp = await fetch("/api/production-execution", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    try {
      const [lotsRes, matsRes, itemsRes, execRes] = await Promise.all([
        fetch("/api/item-production", { cache: "no-store" }).then((r) => r.json()).catch(() => ({})),
        fetch("/api/materials",       { cache: "no-store" }).then((r) => r.json()).catch(() => ({})),
        fetch("/api/items",           { cache: "no-store" }).then((r) => r.json()).catch(() => ({})),
  ...
  return (
    <div>
      <PageHeader
        title="품목 생산"
        description="실 생산 LOT을 기준으로 원료가 차감되고 품목 재고가 증가합니다."
      />
...
{JSON.stringify(processRoundtrip.detail, null, 2)}
      ...
      <Modal open={!!printingLot} onClose={() => setPrintingLot(null)}
        title={printingLot ? `LOT 라벨 미리보기 — ${printingLot.lotCode}` : ""} width="max-w-md"
        footer={<>
