...
import { applyTemplateToItem } from "@/services/itemBom";
import { logWork } from "@/services/history";
import { requireRole } from "@/lib/apiAuth";
import { withErrorHandling } from "@/lib/apiError";
import { SHEET_TABS } from "@/lib/googleSheets";

const TAB = SHEET_TABS.bom;

export async function POST(req: Request) {
  const gate = requireRole("bom");
  if (!gate.ok) return gate.res;
  return withErrorHandling("bom-template apply POST", TAB, async () => {
    ...
    await logWork({
      type: "수정",
      target: `BOM(${itemNo}) ⇐ 템플릿 ${templateName}`,
