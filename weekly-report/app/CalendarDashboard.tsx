...
import {
  Plus, X, ChevronLeft, ChevronRight, AlertTriangle, Circle,
  Trash2, AlertCircle, Flame, User, Tag,
} from "lucide-react";
...
export default function CalendarDashboard({
  ...
  const urgent = useMemo(
    () => schedules.filter((s) => isUrgent(s, today)).sort((a, b) => a.startDate.localeCompare(b.startDate)),
    [schedules, today]);
  const needConfirm = useMemo(
    () => schedules.filter((s) => !s.confirmed).sort((a, b) => a.startDate.localeCompare(b.startDate)),
    [schedules]);
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-semibold text-ink-900 tracking-tight">{formatMonthLabel(viewYear, viewMonth)}</h1>
        ...
        <div className="panel panel-pad mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
            ...
            <div>
              ...
              <input type="date" className="input" value={draft.endDate} min={draft.startDate} onChange={(e) => setDraft((d) => ({ ...d, endDate: e.target.value }))} />
            </div>
            <div>
              <label className="label">담당자</label>
              <input className="input" placeholder="이름" value={draft.assignee} onChange={(e) => setDraft((d) => ({ ...d, assignee: e.target.value }))} />
            </div>
            <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-3">
            <div className="flex-1">
              ...
              <input className="input" placeholder="예: 거래처B 납품, 설비 점검" value={draft.title} autoFocus
            ...
            <label className="flex items-center gap-2 text-sm text-ink-700 pb-2 whitespace-nowrap">
              ...
              <Flame size={14} className="text-status-danger" /> 급함 표시
            </label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_330px] gap-5">
        {/* calendar */}
              ...
              return (
                <button key={iso} onClick={() => openAdd(iso)}
                  ...
                  <div className={`flex items-center justify-center w-6 h-6 rounded-full text-[12px] mb-1
                    ${isToday ? "bg-ink-900 text-bg font-semibold"
        ...
        <div className="space-y-5">
          <div className="panel">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
              ...
              <h2 className="text-sm font-semibold text-ink-900">급한 일정</h2>
              <span className="ml-auto text-xs text-ink-500">{urgent.length}건</span>
            </div>
            <div className="divide-y divide-border">
              {urgent.length === 0 ? (
                <div className="px-4 py-6 text-center text-sm text-ink-400">임박/급한 일정이 없습니다.</div>
              ) : urgent.map((s) => {
                const d = daysBetween(today, s.startDate);
                ...
                return (
                  <div key={s.id} className="px-4 py-2.5 flex items-center gap-2 group">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm text-ink-900">{s.title}</div>
                      <div className="text-[11px] text-ink-500 flex items-center gap-1.5">
                        {s.startDate}{s.assignee && <span className="flex items-center gap-0.5"><User size={10} />{s.assignee}</span>}
                        {s.category && <span className="flex items-center gap-0.5"><Tag size={10} />{s.category}</span>}
                      </div>
                    </div>
                    <button onClick={() => toggleConfirm(s)} title="확인 처리" className="shrink-0 text-ink-300 hover:text-status-done"><Circle size={16} /></button>
                  </div>
                );
              })}
            </div>
          ...
          <div className="panel">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
              <Circle size={15} className="text-beige-500" />
              <h2 className="text-sm font-semibold text-ink-900">확인 필요 일정</h2>
              <span className="ml-auto text-xs text-ink-500">{needConfirm.length}건</span>
            </div>
            <div className="divide-y divide-border">
              {needConfirm.length === 0 ? (
                <div className="px-4 py-6 text-center text-sm text-ink-400">확인할 일정이 없습니다.</div>
              ) : needConfirm.map((s) => (
                <div key={s.id} className="px-4 py-2.5 flex items-center gap-2 group">
                  <button onClick={() => toggleConfirm(s)} title="확인 처리" className="shrink-0 text-ink-300 hover:text-status-done"><Circle size={16} /></button>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm text-ink-900">{s.title}</div>
                    <div className="text-[11px] text-ink-500">{s.startDate}{s.assignee ? ` · ${s.assignee}` : ""}{s.category ? ` · ${s.category}` : ""}</div>
                  </div>
                  <button onClick={() => remove(s)} title="삭제" className="shrink-0 text-ink-300 hover:text-status-danger opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
          </div>

          <p className="text-[11px] text-ink-400 px-1">날짜 칸을 누르면 그 날짜로 일정을 빠르게 추가할 수 있어요. 동그라미를 누르면 ‘확인’ 처리됩니다.</p>
        </div>
      </div>
    </div>
  );
