...
import { consumeMaterials } from "./materials";
import { addItemStock, getItemByNo } from "./items";
...
export async function createItemLot(
  ...
  else getStore().itemLots.unshift(lot);
...
export async function deleteItemLot(id: string): Promise<{ lot: ItemLot }> {
  ...
  const { listExecutionMaterials } = await import("./productionExecution");
